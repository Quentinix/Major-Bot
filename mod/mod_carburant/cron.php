		...
		while ($i_prix < $count_prix) {
			if (@array_search(strtolower($xml_file->pdv[$i]->prix[$i_prix][nom]), $carburant) != FALSE) {
				...
				if ($prix_db != @$xml_file->pdv[$i]->prix[$i_prix][valeur]){
					...
					if ($marque != "") $message = "La station essence ".$marque." à  ".@$xml_file->pdv[$i]->ville." pour le carburant ".@$xml_file->pdv[$i]->prix[$i_prix][nom]." change de prix pour passer de ".$prix_db." Ã  ".@$xml_file->pdv[$i]->prix[$i_prix][valeur].".";
					else $message = "La station essence à ".@$xml_file->pdv[$i]->ville." au ".@$xml_file->pdv[$i]->adresse." pour le carburant ".@$xml_file->pdv[$i]->prix[$i_prix][nom]." change de prix pour passer de ".$prix_db." Ã  ".@$xml_file->pdv[$i]->prix[$i_prix][valeur].".";
